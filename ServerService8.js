(function(Scratch) {
  ...
  // Flag for when a new message has arrived
  let messageEventFlag = false;

  // Keep track of last message and sender for reporters
  let lastMessage = '';
  ...
  class TurboChatExtension {
    ...
    joinServer(args) {
      ...
      socket.onmessage = (event) => {
        ...
        if (data.type === 'message') {
          // Push message into queue with all info
          messageQueue.push({
    ...
    whenMessage() {
      if (messageEventFlag && messageQueue.length > 0) {
        // Pop next message from queue
        const msg = messageQueue.shift();
        lastMessage = msg.content;
        lastMessageSender = msg.username;
        // Reset flag if queue emptied
